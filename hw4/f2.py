...
class SQLManager:
    ...
    def select_data(self, n):
        d = {
            ...
            9: lambda s: s.query(Item.name, Department.sphere).join(Department).all(),
            10: lambda s: s.query(Department.sphere, Item.name).join(Item).all(),
            11: lambda s: s.query(Item.name, Department.sphere).outerjoin(Department).all(),
